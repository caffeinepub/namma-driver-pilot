...
export default function ProfileSetupModal({
  ...
  const { data: existing, isLoading: profileLoading } =
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const profile: UserProfile = {
        principalId:
          existing?.principalId ?? (actor as any).identity?.getPrincipal(),
        ...
        createdTime:
          existing?.createdTime ?? BigInt(Date.now()) * BigInt(1_000_000),
